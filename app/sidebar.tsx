...
const SideBarIconEnd: React.FC<IconProps> = ({ icon, tooltip, ClickFunc }) => (
  <div className="relative group flex justify-center mt-auto">
    ...
    <span className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-max px-2 py-1 rounded bg-text-light text-baselight dark:bg-textdark dark:text-basedark text-xs opacity-0 group-hover:opacity-100 transition pointer-events-none z-10 whitespace-nowrap">
      {tooltip}
    </span>
  </div>
);
...
export default function SideBar() {
  ...
  return (
    <div className="bg-transparent m-0">
      <div className="fixed left-4 top-4 bottom-4 w-16 flex flex-col bg-overlaylight text-textlight dark:bg-overlaydark dark:text-textdark shadow rounded-full">
        <SideBarIcon
        ...
        <span
          ...
          } secret relative flex items-center justify-center h-12 w-12 mb-2 mx-auto mt-auto`}
        >
          <SideBarIconEnd
